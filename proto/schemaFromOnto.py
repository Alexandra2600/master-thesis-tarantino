...
def getPropertiesForClass(g, cat):
  ...
  while to_visit:
    current_cat = to_visit.pop()
    if current_cat in visited:
      continue
    visited.add(current_cat)
    
    for dtp in g.subjects(RDFS.domain, current_cat):
    ...
    for superclass in g.objects(current_cat, RDFS.subClassOf):
...
if __name__ == "__main__":
  neo4j_schema = getSchemaFromOnto("ontos/eventOntology.ttl")
